...
- **Déployer** vers le serveur Foundry (share réseau) : `pwsh ./deploy.ps1` (tous les modules) ou `pwsh ./deploy.ps1 arthaks-table-suite` (un seul). Robocopy en miroir vers `\\192.168.68.59\foundryuserdata\Data\modules`. Puis **F5 dans Foundry** pour recharger.
