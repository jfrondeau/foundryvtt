...
import { TokenHp } from "../lib/token-hp.js";
...
export class CombatOverlay extends FloatingBar {
  ...
  /**
   * Raccourci « / » : édition rapide des PV au clavier. En combat avec un panneau cible
   * affiché, place le focus dans son champ PV (champ AoE partagé, sinon badge PV inline).
   * HORS combat (ou panneau absent), retombe sur le champ PV flottant de TokenHp, ancré au
   * token sélectionné — même moteur d'application. Renvoie true si consommé.
   */
  static focusHp() {
    ...
    // Panneau de combat visible ? On y dirige le focus (AoE partagé, sinon badge PV inline).
    if (inst?.root && inst.root.style.display !== "none") {
    ...
    // Aucun panneau de combat exploitable → champ flottant sur la sélection (hors combat).
    return TokenHp.promptQuickHp();
  }
  ...
  hpColor(actor) {
    if (TokenHp.actorHasDead(actor)) return "#5b6270";
  ...
  combatantDead(c) {
    ...
    return TokenHp.actorHasDead(c.actor);
  }
    ...
    if (victims.length >= 2) {
      ...
      input.addEventListener("keydown", (ev) => {
        if (ev.key === "Enter") {
          ...
          TokenHp.applyHpToVictims(value, victims).finally(() => input.blur());
  ...
  beginHpEdit(token, badge) {
    ...
    input.addEventListener("keydown", (ev) => {
      ...
      if (ev.key === "Enter") {
        ...
        TokenHp.applyHpToOne(token, value);
      } else if (ev.key === "Escape") {
        input.blur();
      }
    });
