# Déploie les modules locaux vers l'instance Foundry LOCALE (cette machine).
# Usage :  pwsh ./deploy-local.ps1                       (déploie tous les modules listés)
#          pwsh ./deploy-local.ps1 arthaks-table-suite   (déploie un seul module)
#          pwsh ./deploy-local.ps1 -Dest "D:\FoundryData\Data\modules"   (userdata ailleurs)
param(
    [string[]]$Modules,
    [string]$Dest = (Join-Path $env:LOCALAPPDATA "FoundryVTT\Data\modules")
)
...
if (-not (Test-Path $destDir)) { throw "Dossier modules introuvable : $destDir (utilise -Dest pour le préciser)" }
